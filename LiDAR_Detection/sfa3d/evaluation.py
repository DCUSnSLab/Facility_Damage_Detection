...
class Losses:
    ...
    def _transpose_and_gather_feat(feat, ind):
        ...
        feat = Losses._gather_feat(feat, ind)
        return feat
    ...
    class FocalLoss(nn.Module):
        ...
        def __init__(self):
            super(Losses.FocalLoss, self).__init__()
            self.neg_loss = Losses._neg_loss
    ...
    class L1Loss(nn.Module):
        def __init__(self):
            super(Losses.L1Loss, self).__init__()

        def forward(self, output, mask, ind, target):
            pred = Losses._transpose_and_gather_feat(output, ind)
    ...
    class L1Loss_Balanced(nn.Module):
        ...
        def __init__(self, alpha=0.5, gamma=1.5, beta=1.0):
            super(Losses.L1Loss_Balanced, self).__init__()
            self.alpha = alpha
            self.gamma = gamma
            assert beta > 0
            self.beta = beta
        ...
        def balanced_l1_loss(self, pred, target):
            ...
            loss = torch.where(diff < self.beta,
                               self.alpha / b * (b * diff + 1) * torch.log(
                                   b * diff / self.beta + 1) - self.alpha * diff,
    ...
    class Compute_Loss(nn.Module):
        def __init__(self, device):
            super(Losses.Compute_Loss, self).__init__()
            self.device = device
            self.focal_loss = Losses.FocalLoss()
            self.l1_loss = Losses.L1Loss()
            self.l1_loss_balanced = Losses.L1Loss_Balanced(alpha=0.5, gamma=1.5, beta=1.0)
        ...
        def forward(self, outputs, tg):
            ...
            loss_stats = {
                'total_loss': torch_utils.to_cpu(total_loss).item(),
                'hm_cen_loss': torch_utils.to_cpu(l_hm_cen).item(),
                'cen_offset_loss': torch_utils.to_cpu(l_cen_offset).item(),
                'dim_loss': torch_utils.to_cpu(l_dim).item(),
                'direction_loss': torch_utils.to_cpu(l_direction).item(),
                'z_coor_loss': torch_utils.to_cpu(l_z_coor).item(),
            }
